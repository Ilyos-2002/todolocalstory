...
function renderingFunc(array, element) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (array[i].isComplate) {
            ...
            // console.log(array[i].isComplate);
        }

        newLi.setAttribute("class", "flex w-[40%] bg-blue-200 py-3 px-2 rounded-lg justify-between gap-16 hover:bg-blue-400");
        newCheckbox.setAttribute("type", "checkbox");
        newCheckbox.setAttribute("class", "w-6 rounded-xl");
        newBtn.setAttribute("class", "py-2 px-4 bg-red-500 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-opacity-75 bg-red-100");
        newCheckbox.dataset.todoId = array[i].id;
        newBtn.dataset.todoId = array[i].id;
        // console.log(newBtn.dataset.todoId);

        newBtn.addEventListener('click', evt => {
            let listItem = evt.target.parentNode
            let listUl = listItem.parentNode;
            // console.log(listItem, listUl);
